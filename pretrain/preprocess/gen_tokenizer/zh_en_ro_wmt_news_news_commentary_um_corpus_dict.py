...
from nmt.preprocess.inputs import zh_en
from lib.preprocess import utils
from lib.utils import create_dir, write_pkl, load_pkl, load_json
from pretrain.preprocess.config import data_dir
from pretrain.preprocess.config import filtered_pos_union_zh_en_dict_path, merged_ro_en_dict_path
...
class GenTokenizer:
    ...
    def __init__(self, data_params={}, preprocess_zh_pl=[], tokenizer_pl=[], _tokenizer_dir='only_news_commentary'):
        ...
        data += self.__load_from_ro_en_dict()
    ...
    @staticmethod
    def __load_from_ro_en():
        loader = ro_en.Loader(0.0, ro_en.Loader.TRAIN_RATIO)
        ro_data, en_data = loader.data()
        return list(zip(ro_data, en_data))
    ...
    @staticmethod
    def __load_from_ro_en_dict():
        # load data from files
        # zh_en_dict = load_json(filtered_pos_union_en_zh_dict_path)
        ro_en_dict = load_json(merged_ro_en_dict_path)
        ro_en_list = list(filter(lambda x: 'translation' in x[1] and x[1]['translation'], ro_en_dict.items()))
        ro_en_list = list(map(lambda x: [[x[0]] * len(x[1]['translation']), x[1]['translation']], ro_en_list))
        # data = reduce(lambda x, y: [x[0] + y[0], x[1] + y[1]], zh_en_list)

        ro_data = []
        en_data = []
        length = len(ro_en_list)
        for i, val in enumerate(ro_en_list):
            if i % 50 == 0:
                progress = float(i + 1) / length * 100.
                print('\rprogress: %.2f%% ' % progress, end='')
    ...
    def __preprocess_zh(self, zh_data):
        return utils.pipeline(self.__preprocess_zh_pl, zh_data, None, self.__data_params)
...
tokenizer_dir = f'zh_en_ro_news_commentary_wmt_news_um_corpus_dict'

GenTokenizer(
    data_params=Model.data_params,
    preprocess_zh_pl=zh_en.seg_zh_by_jieba_pipeline,
